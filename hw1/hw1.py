...
for u in range(i_max):
    plt.xlim(0, N_max)
    sns.scatterplot(x=range(N_max),
                    y=data[:, u],
                    marker=u'|',
                    s=1,
                    color=colors[u])
    plt.savefig('plots/scatter_{}.png'.format(u), facecolor='white')
    plt.show()
...
plt.savefig('plots/blocking.png', facecolor='white')
plt.show()

plt.xlim(0, blocking_n - 1)
for i in range(i_max):
    sns.lineplot(y=blocking_errs[:, i] / blocking_errs[0, i],
                 x=np.arange(blocking_n))
plt.savefig('plots/blocking_norm.png', facecolor='white')
plt.show()
...
plt.savefig('plots/autocorr.png', facecolor='white')
plt.show()
...
plt.savefig('plots/log_autocorr.png', facecolor='white')
plt.show()

plt.xlim(0, corr_len)
for i in range(i_max):
    sns.lineplot(y=autocorr_funcs[:, i].cumsum(),
                 x=np.arange(autocorr_funcs.shape[0]))
plt.savefig('plots/tau', facecolor='white')
plt.show()
...
for l in range(n_blocks):
    jackknife_blocks[l, :] = data[l*block_len:(l+1)*block_len-1, :].sum(0) / \
        block_len
...
ind_err = np.sqrt((autocorr_errs[1:] / means[1:])**2 +
                  (autocorr_errs[0] / means[0])**2) * R
worst_err = np.sqrt(
    (autocorr_errs[1:] / means[1:] + autocorr_errs[0] / means[0])**2) * R
...
n_worst = Kest * (np.sqrt(var_unbiased[1:] / means[1:] +
                          var_unbiased[0] / means[0])) / np.sqrt(N_max)
n_ind = Kest * np.sqrt(var_unbiased[1:] / means[1:]**2 +
                       var_unbiased[0] / means[0]**2) / np.sqrt(N_max)
