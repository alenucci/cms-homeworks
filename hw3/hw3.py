...
types = [("N", i8), ("pos", f8[:, :]), ("vel", f8[:, :]), ("dt", f8),
         ("L", f8), ("r_cut", f8), ("V_rc", f8), ("V_rc_prime", f8)]
    ...
    def _calc_F_U(self, forces, force_indices, part_forces):
        ...
        for part in range(self.N):
            part_forces[part] = forces[force_indices[part, part:]].sum(
                axis=0) - forces[force_indices[part, :part]].sum(axis=0)
        return U
...
if __name__ == "__main__":
    conf = getconf(70)
    MD(*conf).update(0.002, 0.006) # jit compile

    start = time.time()
    MD(*conf).update(0.002, 30.0)
